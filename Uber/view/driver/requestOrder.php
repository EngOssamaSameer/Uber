...
$coldDriverCash = $funds[0]['funds'] ;
$oldOnerCash = $Onerfunds[0]['funds']  ; 
// end order
if(isset($_POST['endRequest'])){
    if($eat->endRequestorder($_POST['endRequest'])){
       $cash->addFundsToDriverAndOner($_SESSION["userId"],$_POST['endRequestCash']+$coldDriverCash ,$oldOnerCash);
    }
 }
  //-----------select all requst  ---------------/
require_once '../../controller/eatController.php';
...
/*  ---------- get Requset ORDER ----------  */
//get order
if(isset($_POST['getRequestOrderId'])){
    $eat->getRequestorder($_SESSION["userId"], $_POST['getRequestOrderId'], $_POST['userId'] ,$_SESSION["userName"] );
...
<body>
    ...
    <main id="main" class="main">
        ...
        <section class="section profile">
            <div class="card">
                <div class="card-body">
                <h5 class="card-title">Request Order</h5>
                        ...
                        <?php
                            if($order){
                                foreach($order as $ord){
                                    ?>
                                        <tr>
                                            <th scope="row"><?php echo $ord['userName'] ?></th>
                                            <th scope="row"><?php echo $ord['userEmail'] ?></th>
                                            <th scope="row"><?php echo $ord['phone'] ?></th>
                                            <th scope="row"><?php echo $ord['address'] ?></th>
                                            <th scope="row"><?php echo $ord['market'] ?></th>
                                            <th scope="row"><?php echo $ord['item'] ?></th>
                                            <th scope="row"><?php echo $ord['cash'] ?></th>
                                            <th scope="row">
                                                <?php
                                                    if($ord['case'] == 1 ){
                                                        ?>
                                                                <form action="requestOrder.php" method="post">
                                                                    <input type="hidden" name="getRequestOrderId"  value="<?php echo $ord['orderId'] ?>">
                                                                    <input type="hidden" name="userId"  value="<?php echo $ord['userId'] ?>">
                                                                    <button type="submit" class="btn btn-success">Get Order</button>
                                                                </form>
                                                        <?php
                                                    }else if($ord['case'] == 2){
                                                        if($ord['driverId'] == $_SESSION["userId"] ){
                                                            ?>
                                                                <form action="requestOrder.php" method="post">
                                                                    <input type="hidden" name="endRequest"  value="<?php echo $ord['orderId'] ?>">
                                                                    <input type="hidden" name="endRequestCash"  value="<?php echo $ord['cash'] ?>">
                                                                    <button type="submit" class="btn btn-danger ">End Order</i></button>
                                                                </form>
                                                            <?php
                                                        }else{
                                                            ?>
                                                                <button type="submit" class="btn btn-warning">Unavalible</button>
                                                            <?php
                                                        }
                                                    }
                                                ?>
